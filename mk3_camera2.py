# in a terminal
# python -m pip install opencv-contrib-python numpy scipy matplotlib ipython jupyter pandas sympy nose imutils
...
showLive = True
while showLive:
    ret, frame = videocapture.read() # grab a frame from the video capture (returns frame=None when it can't grab a frame)
    if frame is None:
        print("ERROR: Couldn't grab frame from video feed!!!!! Potential causes:")
        print("       - Another program is using your camera?")
        print("       - Your camera is broken?")
        print("       - You don't have a camera on your computer?")
    ...
    # From https://opencv-python-tutroals.readthedocs.io/en/latest/py_tutorials/py_imgproc/py_contours/py_contours_begin/py_contours_begin.html
    # im = cv2.imread('test.jpg')
    # imgray = cv2.cvtColor(im,cv2.COLOR_BGR2GRAY)
    # ret,thresh = cv2.threshold(imgray,127,255,0)
    # image, contours, hierarchy = cv2.findContours(thresh,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
    # ... but it looks like this...
    # image, contours, hierarchy = cv2.findContours(closing, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE) # find contours with simple approximation cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE
    contours, hierachy = cv2.findContours(thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE) # ... still doesn't work :,-(
    # Pick back up with https://stackoverflow.com/questions/54164630/want-to-find-contours-valueerror-not-enough-values-to-unpack-expected-3-go
    # and googling ferociusly.
